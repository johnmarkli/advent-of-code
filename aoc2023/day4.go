...
func (c *card) numMatches() int {
	if c.matches == nil {
		count := 0
		winningNumMap := map[int]bool{}
		for _, num := range c.winningNums {
			winningNumMap[num] = true
		}
		// loop player nums and update score if match
		for _, num := range c.playerNums {
			if _, ok := winningNumMap[num]; ok {
				count++
			}
		}
		c.matches = &count
...
func Day4Part1(filepath string) any {
	...
	cards := parseCards(fileScanner)
	// for each card
	for _, card := range cards {
		// create map of winning nums
		score := 0
		winningNumMap := map[int]bool{}
		...
		// loop player nums and update score if match
		for _, num := range card.playerNums {
			if _, ok := winningNumMap[num]; ok {
				if score == 0 {
					score = 1
				} else {
...
func Day4Part2(filepath string) any {
	...
	cards := parseCards(fileScanner)
	// origCards := []card{}
	// origCards = append(origCards, cards...)
	...
	// while pointer less than len(cards)
	for i < len(cards) {
		// get number of matches for card
		numMatches := cards[i].numMatches()
		// append cards to cards
		for j := 1; j <= numMatches; j++ {
			x := cards[i].num - 1
			cards = append(cards, cards[x+j])
			numCards[cards[x+j].num]++
		}
		// cardNums := []int{}
		// for _, card := range cards {
		// 	cardNums = append(cardNums, card.num)
		// }
...
func parseCards(fileScanner *bufio.Scanner) []card {
	cards := []card{}
	for fileScanner.Scan() {
		line := fileScanner.Text()
		split1 := strings.Split(line, ": ")
		cardNumStr := strings.Fields(split1[0])[1]
		cardNum, _ := strconv.Atoi(cardNumStr)
		if len(split1) == 2 {
			split2 := strings.Split(split1[1], " | ")
			if len(split2) == 2 {
				winningNumStr := strings.Fields(split2[0])
				winningNums := []int{}
				for _, str := range winningNumStr {
				...
				playerNumStr := strings.Fields(split2[1])
				playerNums := []int{}
				for _, str := range playerNumStr {
					num, err := strconv.Atoi(str)
					if err != nil {
						panic(err)
					}
