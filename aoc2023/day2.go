...
func Day2Part1(filepath string) any {
	...
	for fileScanner.Scan() {
		...
		gameNum, err := strconv.Atoi(strings.Split(split[0], " ")[1])
		if err != nil {
			panic(err)
		}
		...
		for _, set := range sets {
			// parse out number of each colour and compare with maxCubes
			...
			for _, cube := range cubes {
				...
				// if doesn't exceed max, add game number to result
