...
func Day3Part1(filepath string) any {
	...
	for fileScanner.Scan() {
		...
		halfMark := len(line) / 2
		half1, half2 := line[:halfMark], line[halfMark:]

		// find duplicate in each half
		seen := map[rune]bool{}
		// loop first half and create a map
		for _, char := range half1 {
			seen[char] = true
		}

		var dup rune
		// loop second half and if found in map, break and use char
		for _, char := range half2 {
		...
		if dup == 0 {
			fmt.Println("no dup", line, halfMark, half1, half2)
		}

		// convert dup char to priority num and add to sum
		sum += priority(dup)
	}
...
func Day3Part2(filepath string) any {
	...
	// loop bag 3 to see if in bag1 and bag2
	sum := 0
	...
	for fileScanner.Scan() {
		...
		if count%3 == 0 && len(group) == 3 { // every 3rd line find common letter in group then reset group
			// create seen maps for all lines except last
			seen1 := map[rune]bool{}
			...
			// loop last line and find in others
			var badge rune
			for _, char := range group[2] {
				if _, ok := seen1[char]; ok {
					if _, ok2 := seen2[char]; ok2 {
						badge = char
						break
					}
				}
