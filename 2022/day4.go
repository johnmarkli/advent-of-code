...
func Day4Part1(filepath string) any {
	...
	for fileScanner.Scan() {
		...
		firstRange := getAssignedRange(first)
		secondRange := getAssignedRange(second)
...
func getAssignedRange(s string) assignedRange {
	split := strings.Split(s, "-")
	l, _ := strconv.Atoi(split[0])
	h, _ := strconv.Atoi(split[1])
	return assignedRange{l, h}
}

func covered(first assignedRange, second assignedRange) bool {
	// if first low >= second low and first high <= second high OR
...
b  b
*/
func overlap(first assignedRange, second assignedRange) bool {
	// if first high >= second low OR second high >= first low OR first low >= second high OR second low >= first high
	return (first.high >= second.low && first.high <= second.high) ||
