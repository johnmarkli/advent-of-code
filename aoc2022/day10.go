...
complete. It has no other effect. The CPU uses these instructions in a program
...
func Day10Part1(filepath string) any {
	...
	// queue up operations to be completed on certain cycles

	// keep count of cycles
	// for each instruction, update cycle number and operation on register x
	// if cycle is 20th, 60th, etc. then add (cycle number * x) to res

	// init cycle number
	cycle := 0

	// init register value x to 1
	x := 1
	...
	for fileScanner.Scan() {
		...
		case "noop":
			...
			if cycle == 20 || cycle == 60 || cycle == 100 || cycle == 140 || cycle == 180 || cycle == 220 {
				signal := cycle * x
				res += signal
				// fmt.Println(instr, param, cycle, x, signal)
			}
		case "addx":
			cycle++
			if cycle == 20 || cycle == 60 || cycle == 100 || cycle == 140 || cycle == 180 || cycle == 220 {
				signal := cycle * x
				res += signal
				// fmt.Println("mid", instr, param, cycle, x, signal)
			}
			cycle++
			if cycle == 20 || cycle == 60 || cycle == 100 || cycle == 140 || cycle == 180 || cycle == 220 {
				signal := cycle * x
				res += signal
				// fmt.Println("end", instr, param, cycle, x, signal)
			}
			x += param
		}
	}
